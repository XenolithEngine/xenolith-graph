...
XenolithGraph ships a **Svelte adapter** (`@xenolithengine/svelte`) and a **framework-agnostic Web Component** (`@xenolithengine/wc`). Both run only in the browser, so SvelteKit needs to be told to skip them on the server.

## Install

```sh
pnpm add @xenolithengine/editor @xenolithengine/svelte pixi.js
# or, for the framework-agnostic web-component path:
pnpm add @xenolithengine/editor @xenolithengine/wc pixi.js
```
...
<script lang="ts">
  ...
  onMount(async () => {
    if (!browser) return
    const { register } = await import('@xenolithengine/wc')
