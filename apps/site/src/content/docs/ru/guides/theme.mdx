...
{/* EN-canonical-banner */}
English is the canonical source. If something contradicts the [English version](../../../guides/theme/) — trust the English page. We'll re-sync translations periodically until v1.0.
:::
...
Оригинальная dark/gold дизайн-система: тела нод `#0F110E`, золотые акценты, тонированные по категориям шапки, типизированные провода, анимированный коллапс в pill. Живёт в `@xenolithengine/theme-xen`.
...
Приоритет цвета шапки ноды: **`render.color` → `categories[category]` → токен темы `category` → fallback `utility`**. Оба поля необязательны — графы без них рисуются как раньше, через встроенные категории темы. Типы `CategoryColorSpec` / `GraphCategoryPalette` экспортируются из `@xenolithengine/render-pixi`.
...
Полный тип лежит в `@xenolithengine/theme-xen` и называется `XenTokens`. Верхнеуровневые группы:
...
import { mergeTheme } from '@xenolithengine/theme-xen'
...
Для кастомного материала (свой шейдер, свой backdrop) — реализуйте `renderNode` и опционально `createGrid`. Полный пример см. в исходниках `@xenolithengine/theme-liquid-glass` — PIXI Mesh + GLSL с SDF-рефракцией и сэмплингом backdrop.
