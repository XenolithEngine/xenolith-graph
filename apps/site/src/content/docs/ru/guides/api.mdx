...
Выполнение делегируется плагинам — сам редактор **не содержит execution**. Рантайм (например `@xenolithengine/graph-plugin-runtime`, Blueprint VM, в работе) ставится плагином и использует [API делегирования рантайма](#плагины) выше.
