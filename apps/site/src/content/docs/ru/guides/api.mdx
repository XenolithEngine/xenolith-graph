...
{/* EN-canonical-banner */}
English is the canonical source. If something contradicts the [English version](../../../guides/api/) — trust the English page. We'll re-sync translations periodically until v1.0.
:::
...
Выполнение делегируется плагинам — сам редактор **не содержит execution**. Рантайм (например `@xenolithengine/plugin-runtime`, Blueprint VM, в работе) ставится плагином и использует [API делегирования рантайма](#плагины) выше.

```ts
import { runtime } from '@xenolithengine/plugin-runtime'   // будущий пакет
