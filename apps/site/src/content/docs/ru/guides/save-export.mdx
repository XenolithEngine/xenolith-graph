...
{/* EN-canonical-banner */}
English is the canonical source. If something contradicts the [English version](../../../guides/save-export/) — trust the English page. We'll re-sync translations periodically until v1.0.
:::
...
The `@xenolithengine/runtime` package ships a workflow importer that consumes the JSON ComfyUI writes when you save a workflow:

```ts
import { importComfyWorkflow } from '@xenolithengine/runtime/comfy'
