...
The React adapter ships as `@xenolith/react`. It is a thin layer over the core editor — every
...
```bash
pnpm add @xenolith/react @xenolith/editor pixi.js
```
...
```tsx
import { reactWidget, type WidgetProps } from '@xenolith/react'
...
```tsx
import { useXenolith } from '@xenolith/react'
...
- [`@xenolith/editor` API reference](/guides/api/) — every method exposed via `useEditor()`
- [Widgets guide](/guides/widgets/) — built-in widgets + the `WidgetSpec` shape `reactWidget` plugs into
- [Save / export](/guides/save-export/) — `toJSON` / `loadJSON` / `exportImage` patterns
- [Events](/guides/events-commands/) — the full 24-event surface and which are preventable
