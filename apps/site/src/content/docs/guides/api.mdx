...
Execution is delegated to plugins — the editor itself ships **no execution**. A runtime (e.g. `@xenolithengine/plugin-runtime`, a Blueprint VM, in progress) installs as a plugin and uses the [runtime delegation API](#plugins) above.

```ts
import { runtime } from '@xenolithengine/plugin-runtime'   // future package
