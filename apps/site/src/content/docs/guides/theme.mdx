...
Anything **not** in the map still falls back to Google Fonts — so partial self-host is allowed (cover just the weights you have local). To download Inter WOFF2 binaries, grab them from <https://fonts.google.com/specimen/Inter> and host alongside your app assets.
