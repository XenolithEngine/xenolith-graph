...
That's all. `@xenolithengine/editor` lists `core`, `render-pixi`, and `theme-xen` as transitive dependencies — npm pulls them automatically. `pixi.js` stays a peer dependency so you pin it yourself and keep control of the WebGL renderer version.
