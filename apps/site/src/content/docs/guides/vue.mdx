...
The full mapping lives in `EDITOR_EVENT_NAMES` from `@xenolithengine/graph-adapter-core`. Preventable events
...
- [`@xenolithengine/graph-editor` API reference](../api/) — every method exposed by `useEditor()` / `useEditorReady()`
