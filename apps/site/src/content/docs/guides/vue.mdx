...
The Vue adapter ships as `@xenolith/vue`. Composition-API only — there is no Options-API mirror.
...
```bash
pnpm add @xenolith/vue @xenolith/editor pixi.js vue
```
...
The full mapping lives in `EDITOR_EVENT_NAMES` from `@xenolith/adapter-core`. Preventable events
...
```vue
<script setup lang="ts">
import { XenolithPanel, XenolithButton, useUndoRedo } from '@xenolith/vue'
...
<script setup lang="ts">
import type { WidgetProps } from '@xenolith/vue'
...
import { vueWidget } from '@xenolith/vue'
...
a portal? Use `createEditorBinding` from `@xenolith/adapter-core` directly:
...
- [`@xenolith/editor` API reference](/guides/api/) — every method exposed by `useEditor()` / `useEditorReady()`
- [Widgets guide](/guides/widgets/) — built-in widgets + the `WidgetSpec` shape `vueWidget` plugs into
