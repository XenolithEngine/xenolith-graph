...
执行被委托给插件——编辑器自身**不附带执行**。运行时（如 `@xenolithengine/plugin-runtime`、Blueprint VM，正在开发中）作为插件安装，使用上面的[运行时委托 API](#插件)。

```ts
import { runtime } from '@xenolithengine/plugin-runtime'   // 未来包
