...
A more featured executor ships in [`@xenolithengine/plugin-runtime`](https://github.com/XenolithEngine/xenolith-graph/tree/main/packages/plugin-runtime) — type-aware, async-aware, with three engines (interpreted, codegen-JS, AssemblyScript-WASM). It's the same shape, just dialled up.
