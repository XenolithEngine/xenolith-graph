...
> **Experimental** — supports the numeric subset only (`canCompileToAS(graph)` gates eligibility). Non-numeric primitives fall back to `@xenolithengine/graph-plugin-runtime`'s JS codegen.
...
```bash
pnpm add @xenolithengine/graph-runtime-as @xenolithengine/graph-plugin-runtime
```
