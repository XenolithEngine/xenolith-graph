...
> **Experimental** — supports the numeric subset only (`canCompileToAS(graph)` gates eligibility). Non-numeric primitives fall back to `@xenolithengine/plugin-runtime`'s JS codegen.
...
```bash
pnpm add @xenolithengine/runtime-as @xenolithengine/plugin-runtime
```
...
```ts
import { compile, canCompileToAS } from '@xenolithengine/runtime-as'
import { BUILTIN_PRIMITIVES, type RtGraph } from '@xenolithengine/plugin-runtime'
