...
The default theme is **Xen**, an original design system. The package is `@xenolithengine/graph-theme-xen`. The Figma source is the canonical authority.
...
- `@xenolithengine/graph-theme-ue5` and `@xenolithengine/graph-theme-ue4` are removed from the package map. Future alternative themes — light, high-contrast, brand-customised — live under `@xenolithengine/graph-theme-*` post-1.0.
